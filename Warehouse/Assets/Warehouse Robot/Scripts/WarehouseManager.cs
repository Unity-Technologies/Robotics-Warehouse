...
namespace Unity.Simulation.Warehouse {
    ...
    {
        [Header("Run Generation and NavMeshAgent bot?")]
        public bool m_generate = false;
        public bool m_navMeshBotOn = true;
        ...
        void Awake()
        {
            ...
            // Spawn bots
            if (m_navMeshBotOn){
                for (int i = 1; i < appParam.m_numBots; i++){
        ...
        private List<GameObject> GenerateShelves(){
            ...
            if (!m_navMeshBotOn) m_roadPrefab = new GameObject("PathLocation");
            ...
            h = Instantiate(m_roadPrefab, new Vector3(0, _pathHeight, appParam.m_length /2 - (r/2)), _hRot, pathParent);
            h.transform.localScale = new Vector3(c / 30f, h.transform.localScale.y, appParam.m_width / 10f);
            if (m_navMeshBotOn) h.GetComponent<NavMeshSurface>().BuildNavMesh();
            _paths.Add(v);
            _paths.Add(h);

            if (!m_navMeshBotOn) Destroy(m_roadPrefab);
        ...
        private List<Vector3> GenerateSleepPosition(){
            ...
            while (cur.x < (appParam.m_width / 2) - botSize.x * 2){
